...
export const createMessage = base
  ...
  .handler(async ({ input, context, errors }) => {
    //verify the cahnnel belongs to the users's organization
  ...
  if(!parentMessage || parentMessage.channelId !== input.channelId || parentMessage.threadId !== null){
    ...
    const created = await prisma.message.create({
      data: {
        ...
        authorName: context.user.given_name ?? "shankar-webdev",
...
export const listMessages = base
  ...
  // reactions: groupReactions(m.MessageReaction, context.user.id)
  ...
  export const updateMessage = base.use(requiredAuthMiddleware)
  .use(requiredWorkspaceMiddleware)
  .use(standardSecurityMiddleware)
  .use(WriteSecurityMiddleware)
  .route({
...
export const listThreadReplies = base.use(requiredAuthMiddleware).use(requiredWorkspaceMiddleware).use(standardSecurityMiddleware).use(readSecurityMiddleware).route({
  method:'GET',
...
export const toggleReaction = base.use(requiredAuthMiddleware)
.use(requiredWorkspaceMiddleware)
.use(standardSecurityMiddleware)
.use(WriteSecurityMiddleware)
.route({
      ...
      data:[
        {
          ...
          userName: context.user.given_name?? "shankar_webDev",
