...
export const generateThreadSummary = base
  ...
  .handler(async ({ input, context, errors }) => {
    ...
    lines.push(
      `Thread Root -${parent.authorName} -${parent.createdAt.toString()}`
    );
...
export const generateCompose = base
  ...
  .handler(async ({ input }) => {
    ...
    //Focused system prompt for deterministic rewrite(non-converstional)
    const system = [
    ...
    const result = streamText({
      ...
      messages: [
        {
          role: "user",
          content: "Plese rewrite and imporve the following content: ",
        },
        {
          role: "user",
          content: markdown,
